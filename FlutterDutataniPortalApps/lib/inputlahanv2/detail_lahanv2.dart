...
class DetailLahanV2 extends StatefulWidget {
  final VoidCallback reload;
  final Lahan lahan;

  const DetailLahanV2({Key key,@required this.lahan, this.reload}): super(key:key);
  @override
  _DetailLahanState createState() => _DetailLahanState();
}

class _DetailLahanState extends State<DetailLahanV2> {
  ...
  simpanData() async{
    ...
    body: {
      ...
      "lat" : _markers.elementAt(_markers.length-1).position.latitude.toString(),
      "longt" : _markers.elementAt(_markers.length-1).position.longitude.toString()
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        centerTitle: true,
        elevation: 0.0,
        title: Text("DETAIL LAHAN V2 ID "+widget.lahan.ID_Lahan),
      ),
      body:
          Column(
            children: <Widget>[
